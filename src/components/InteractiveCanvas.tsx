...
export function InteractiveCanvas({ 
  ...
  // Ölçekleme Tamiri: naturalPixelsPerInch Analizden (3-6) gelmeli. 
  // Çok büyükse poster odayı kaplar. safePPI ile sınırlıyoruz.
  const safePPI = Math.min(Math.max(naturalPixelsPerInch || 4, 3), 6);
      ...
      >
        {mountedArt ? (
          ...
          <div className="flex flex-col items-center gap-2 opacity-10">
            ...
            <span className="text-[8px] uppercase font-bold tracking-widest">Empty Frame</span>
          </div>
        )}
      </motion.div>
    </div>
