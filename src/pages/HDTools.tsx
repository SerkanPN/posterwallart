...
export function HDTools() {
  ...
  const { getRootProps, getInputProps } = useDropzone({ onDrop, accept: { 'image/*': [] as string[] }, maxFiles: 1 } as any);
            ...
            {activeTab === 'gen' ? (
              <div className="space-y-6">
                <div className="space-y-3">
                  ...
                  <select value={selectedModel.id} onChange={(e) => setSelectedModel(CUSTOM_MODELS.find(m => m.id === e.target.value) || CUSTOM_MODELS[0])} className="w-full bg-zinc-950 border border-zinc-800 p-4 rounded-2xl text-xs outline-none focus:border-indigo-500 transition-colors">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex gap-2">
                    <select value={selectedLora} onChange={(e) => setSelectedLora(e.target.value)} className="flex-1 bg-zinc-950 border border-zinc-800 p-4 rounded-2xl text-xs outline-none focus:border-indigo-500 transition-colors">
                ...
                <div className="space-y-3">
                  ...
                  <textarea value={prompt} onChange={(e) => setPrompt(e.target.value)} placeholder="Describe the masterpiece..." className="w-full bg-zinc-950 border border-zinc-800 p-4 rounded-2xl text-xs outline-none h-32 focus:border-indigo-500 resize-none transition-colors" />
                </div>
                <button onClick={handleGenerate} disabled={isProcessing} className="w-full py-4 bg-indigo-600 hover:bg-indigo-500 text-white font-black uppercase rounded-2xl transition-all shadow-[0_0_30px_rgba(99,102,241,0.2)] disabled:opacity-50">
                ...
                <div key={res.id} className="bg-zinc-900 border border-zinc-800 p-4 rounded-3xl flex gap-4 items-center animate-in fade-in slide-in-from-bottom-4 duration-500">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded-md ${res.type === 'gen' ? 'bg-indigo-500/20 text-indigo-400' : res.type === 'vector' ? 'bg-orange-500/20 text-orange-400' : 'bg-blue-500/20 text-blue-400'}`}>
                        {res.type}
                      </span>
                    ...
                    <a href={res.url} download={`${res.type}_result.${res.type === 'vector' ? 'svg' : 'png'}`} className="inline-flex items-center gap-1.5 text-[10px] text-zinc-100 font-black uppercase mt-2 border-b border-zinc-700 pb-0.5 hover:text-indigo-400 hover:border-indigo-400 transition-all">
