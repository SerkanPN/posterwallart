  ...
  useEffect(() => {
    ...
    const initApp = async () => {
      ...
      w.handleStartTimeChange = function() {
        ...
        document.getElementById('time-start-el')!.textContent = startStr;
        const s = w.timeToSeconds(startStr); const e = w.timeToSeconds(endStr);
        if (e > 0 && s <= e) {
          let pct = (s / e) * 100; if (pct < 0) pct = 0; if (pct > 100) pct = 100;
          (document.getElementById('progress-val') as HTMLInputElement).value = pct.toString();
      ...
      w.handleEndTimeChange = function() {
        document.getElementById('time-end-el')!.textContent = (document.getElementById('time-end') as HTMLInputElement).value;
        w.handleStartTimeChange(); 
      };

      w.updateProgress = function(val: string) {
        document.getElementById('progress-bar-fill')!.style.width = val + '%';
        document.getElementById('progress-display')!.textContent = Math.round(parseFloat(val)) + '%';
        const e = w.timeToSeconds((document.getElementById('time-end') as HTMLInputElement).value);
        if (e > 0) {
          const newTimeStr = w.secondsToTime((parseFloat(val) / 100) * e);
          (document.getElementById('time-start') as HTMLInputElement).value = newTimeStr;
          document.getElementById('time-start-el')!.textContent = newTimeStr;
        }
      };
      ...
      w.updateBgBlur = function() {
        ...
        if (!blurEl || !w.currentCoverSrc) return;
        ...
        if(document.getElementById('brightness-display')) document.getElementById('brightness-display')!.textContent = brightness + '%';
        ...
        tempImg.onload = () => {
          ...
          try {
            ...
            document.getElementById('poster-bg-img')!.style.backgroundImage = `url(${dataUrl})`;
          } catch (e) {}
        };
      ...
      w.updateBg = function() {
        ...
        if (type === 'color') {
          ...
          document.getElementById('bg-blur-section')!.style.display = 'none';
          document.getElementById('poster-bg-img')!.style.display = 'none';
          w.updateBgColor();
        } else {
          document.getElementById('bg-color-section')!.style.display = 'none';
          document.getElementById('bg-blur-section')!.style.display = 'block';
          document.getElementById('poster-bg')!.style.background = 'none';
          document.getElementById('poster-bg-img')!.style.display = w.currentCoverSrc ? 'block' : 'none';
          w.updateBgBlur();
        }
      };

      w.updateOverlay = function() {
        const val = (document.getElementById('overlay-val') as HTMLInputElement).value;
        document.getElementById('overlay-display')!.textContent = val + '%';
        const overlay = document.getElementById('poster-bg-overlay')!;
        overlay.style.display = 'block'; overlay.style.background = `rgba(0,0,0,${parseFloat(val)/100})`;
      };
      ...
      w.setCoverImage = function(src: string) {
        ...
        if(img) { img.src = src; img.style.display = 'block'; if(placeholder) placeholder.style.display = 'none'; }
        ...
        if(bgImgDiv) {
            ...
            if (btype && btype.value === 'blur') { bgImgDiv.style.display = 'block'; w.updateBgBlur(); }
        }
      ...
      w.handleCoverUpload = function(event: any) {
        ...
        const reader = new FileReader(); reader.onload = (e: any) => w.setCoverImage(e.target.result); reader.readAsDataURL(file);
      };
      ...
      w.updateTextColor = function(id: string, color: string) {
        const el = document.getElementById(id); if (el) el.style.color = color;
      };
      ...
      w.syncColor = function(colorId: string, textId: string) {
        ...
        if (/^#[0-9A-Fa-f]{6}$/.test(txt)) { colEl.value = txt; colEl.dispatchEvent(new Event('input', { bubbles: true })); colEl.dispatchEvent(new Event('change', { bubbles: true })); }
      };

      w.currentTrackUri = ''; w.cachedSvgText = null;
      w.fetchSvgRaw = async function() {
        ...
        try { const res = await fetch(url); if (!res.ok) return null; const text = await res.text(); if (text.includes('<svg')) return text; } catch(e) {}
        return null;
      };
      ...
      w.renderBarcodeFromCache = function() {
        ...
        const parser = new DOMParser(); const doc = parser.parseFromString(w.cachedSvgText, 'image/svg+xml'); const fetched = doc.querySelector('svg');
        if (!fetched) return;
        Array.from(fetched.querySelectorAll('rect')).forEach((r, i) => { if (i === 0) r.remove(); else r.setAttribute('fill', barColor); });
        fetched.querySelectorAll('circle').forEach(el => el.setAttribute('fill', logoColor));
        fetched.querySelectorAll('path').forEach(el => { const fill = el.getAttribute('fill'); if (fill && fill !== 'none') el.setAttribute('fill', logoColor); });
        const svg = document.getElementById('barcode-svg');
        if(svg){ svg.setAttribute('viewBox', fetched.getAttribute('viewBox') || '0 0 400 100'); svg.innerHTML = fetched.innerHTML; }
      };
      ...
      w.drawBarcode = async function() {
        ...
        if (!w.cachedSvgText) { if(svg) svg.innerHTML = `<text x="50%" y="50%" text-anchor="middle" fill="#ff4444" font-size="10" dy=".3em">Barcode error</text>`; return; }
      ...
      w.handleSpotifyInput = function() {
        ...
        let uri = null;
        const urlMatch = input.match(/track\/([a-zA-Z0-9]+)/); if (urlMatch) uri = `spotify:track:${urlMatch[1]}`;
        const uriMatch = input.match(/spotify:track:([a-zA-Z0-9]+)/); if (uriMatch) uri = input;
        if (uri) { w.currentTrackUri = uri; w.cachedSvgText = null; w.drawBarcode(); }
      };

      w.updateBarcodeStyle = function() { if (w.cachedSvgText) w.renderBarcodeFromCache(); else if (w.currentTrackUri) w.drawBarcode(); };

      w.showToast = function(msg: string) { const toast = document.getElementById('toast'); if(!toast) return; toast.textContent = msg; toast.classList.add('show'); setTimeout(() => toast.classList.remove('show'), 3500); };

      w.searchSong = async function() {
        const q = (document.getElementById('search-input') as HTMLInputElement).value.trim(); if (!q) return;
        const spinner = document.getElementById('search-spinner'); const results = document.getElementById('search-results');
        if(spinner) spinner.style.display = 'block'; if(results) results.style.display = 'none';
        try {
        ...
        } catch(e: any) { if(spinner) spinner.style.display = 'none'; w.showToast('Arama hatası: ' + e.message); }
      };

      w.applySearchResult = async function(item: any) {
        const stitle = document.getElementById('song-title'); const sartist = document.getElementById('song-artist');
        if(stitle) stitle.textContent = item.trackName; if(sartist) sartist.textContent = item.artistName;
        const tinp = document.getElementById('song-title-input') as HTMLInputElement; const ainp = document.getElementById('song-artist-input') as HTMLInputElement;
        if(tinp) tinp.value = item.trackName; if(ainp) ainp.value = item.artistName;
        if (item.trackTimeMillis) {
          const endStr = w.secondsToTime(item.trackTimeMillis / 1000);
          const tend = document.getElementById('time-end') as HTMLInputElement; if(tend) tend.value = endStr;
          const tel = document.getElementById('time-end-el'); if(tel) tel.textContent = endStr;
          w.handleStartTimeChange();
        }
        w.setCoverImage(item.artworkUrl100.replace('100x100bb', '600x600bb'));
      ...
      w.generateSafeCanvas = async function(container: HTMLElement, scale: number) {
        ...
        try {
          const generatedCanvas = await w.html2canvas(container, { scale: scale, useCORS: true, allowTaint: true, backgroundColor: document.getElementById('poster-bg')?.style.background || '#121212', scrollX: 0, scrollY: 0 });
      ...
      w.downloadPNG = async function() {
        w.edDeselect(); w.showToast('PNG hazırlanıyor...');
        const container = document.getElementById('poster-container');
        const key = (document.getElementById('canvas-size') as HTMLSelectElement).value; const [wIn, hIn] = w.CANVAS_SIZES[key];
        let targetWidthPx = Math.round(wIn * 300); let targetHeightPx = Math.round(hIn * 300); let scale = targetWidthPx / container!.offsetWidth;
        const MAX_DIMENSION = 16000;
        if (targetWidthPx > MAX_DIMENSION || targetHeightPx > MAX_DIMENSION) { const limitScale = Math.min(MAX_DIMENSION / targetWidthPx, MAX_DIMENSION / targetHeightPx); scale = scale * limitScale; }
        try { const canvas = await w.generateSafeCanvas(container, scale); const base64Data = canvas.toDataURL('image/png', 1.0); const dpiFixedData = w.changeDpiDataUrl(base64Data, 300); const link = document.createElement('a'); link.download = w.getExportFilename('png'); link.href = dpiFixedData; link.click(); w.showToast('✓ PNG indirildi!'); } catch(e: any) { w.showToast('Hata: ' + e.message); }
      };

      w.downloadPDF = async function() {
        w.edDeselect(); w.showToast('PDF hazırlanıyor...');
        const container = document.getElementById('poster-container');
        const key = (document.getElementById('canvas-size') as HTMLSelectElement).value; const [wIn, hIn] = w.CANVAS_SIZES[key];
        let targetWidthPx = Math.round(wIn * 300); let targetHeightPx = Math.round(hIn * 300); let scale = targetWidthPx / container!.offsetWidth;
        const MAX_DIMENSION = 16000;
        if (targetWidthPx > MAX_DIMENSION || targetHeightPx > MAX_DIMENSION) { const limitScale = Math.min(MAX_DIMENSION / targetWidthPx, MAX_DIMENSION / targetHeightPx); scale = scale * limitScale; }
        try { const canvas = await w.generateSafeCanvas(container, scale); const { jsPDF } = w.jspdf; const pdf = new jsPDF({ orientation: wIn > hIn ? 'landscape' : 'portrait', unit: 'in', format: [wIn, hIn] }); const imgData = canvas.toDataURL('image/jpeg', 0.95); pdf.addImage(imgData, 'JPEG', 0, 0, wIn, hIn); pdf.save(w.getExportFilename('pdf')); w.showToast('✓ PDF indirildi!'); } catch(e: any) { w.showToast('Hata: ' + e.message); }
      };

      w.downloadSVG = async function() {
        w.edDeselect(); w.showToast('SVG hazırlanıyor...');
        const container = document.getElementById('poster-container');
        const key = (document.getElementById('canvas-size') as HTMLSelectElement).value; const [wIn, hIn] = w.CANVAS_SIZES[key];
        let targetWidthPx = Math.round(wIn * 300); let targetHeightPx = Math.round(hIn * 300); let scale = targetWidthPx / container!.offsetWidth;
        const MAX_DIMENSION = 16000;
        if (targetWidthPx > MAX_DIMENSION || targetHeightPx > MAX_DIMENSION) { const limitScale = Math.min(MAX_DIMENSION / targetWidthPx, MAX_DIMENSION / targetHeightPx); scale = scale * limitScale; targetWidthPx = Math.round(targetWidthPx * limitScale); targetHeightPx = Math.round(targetHeightPx * limitScale); }
        try { const canvas = await w.generateSafeCanvas(container, scale); const imgData = canvas.toDataURL('image/png', 1.0); const svgContent = `<?xml version="1.0" encoding="UTF-8"?>\n<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="${targetWidthPx}" height="${targetHeightPx}" viewBox="0 0 ${targetWidthPx} ${targetHeightPx}">\n  <image href="${imgData}" x="0" y="0" width="${targetWidthPx}" height="${targetHeightPx}"/>\n</svg>`; const blob = new Blob([svgContent], { type: 'image/svg+xml' }); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.download = w.getExportFilename('svg'); link.href = url; link.click(); URL.revokeObjectURL(url); w.showToast('✓ SVG indirildi!'); } catch(e: any) { w.showToast('Hata: ' + e.message); }
      };

      w.ED_LABELS = { 'label-top': 'Top Label', 'cover': 'Cover Art', 'song-info': 'Song Info Group', 'song-title': 'Song Title', 'song-artist':'Artist Name', 'heart': 'Heart Icon', 'progress': 'Progress Bar', 'play': 'Play Button', 'btn-shuffle':'Shuffle Button', 'btn-prev': 'Previous Button', 'btn-next': 'Next Button', 'btn-repeat': 'Repeat Button', 'barcode': 'Spotify Barcode' };
      ...
      w.edSetText = function(id: string, val: string) {
        ...
        const map: any = { 'song-title': 'song-title-input', 'song-artist': 'song-artist-input', 'label-top': 'label-top-input' };
  ...
  return (
    <div className="spotify-poster-page">
      ...
      <div id="panel">
        ...
        <button className="accordion-btn" onClick={(e) => (window as any).toggleAccordion(e.currentTarget)}>⏱️ Progress & Controls<span className="arrow">▼</span></button>
        <div className="accordion-content">
          <div className="toggle-row">
            <label>Progress Visible</label>
              ...
              <div id="poster-content">
                <div id="spotify-card" style={{ display: 'flex' }}>
                  ...
                  <div id="cover-wrapper" className="ed-el" data-ed="cover">
                    <div id="cover-placeholder">COVER</div>
                  ...
                  <div id="controls-row">
                    <button className="ctrl-btn ed-el" data-ed="btn-prev">P</button>
                    <button id="play-btn" className="ed-el" data-ed="play">PLAY</button>
                    <button className="ctrl-btn ed-el" data-ed="btn-next">N</button>
                  </div>
