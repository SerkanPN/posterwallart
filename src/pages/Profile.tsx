            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-0">
              ...
              <div className="p-8 flex flex-col gap-6">
                ...
                <div className="bg-indigo-500/5 border border-indigo-500/20 rounded-2xl p-4">
                  <p className="text-[9px] text-indigo-400 font-black uppercase tracking-widest mb-2">Your Creation · Free Download</p>
                  <button
                    onClick={() => handleDownload(selectedDesign)}
                    disabled={processingId === String(selectedDesign.id)}
                    className="w-full py-3 bg-indigo-600 hover:bg-indigo-500 text-white text-[10px] font-black uppercase rounded-xl flex items-center justify-center gap-2 transition-all disabled:opacity-50"
                  >
                  ...
                  <div className="grid grid-cols-2 gap-2 max-h-48 overflow-y-auto pr-1">
                    {VARIANTS.map(v => (
                      <button
                        ...
                        className={`p-3 rounded-xl border text-left transition-all ${selectedVariant.id === v.id ? 'border-indigo-500 bg-indigo-500/5' : 'border-zinc-800 bg-zinc-950 hover:border-zinc-700'}`}
                      >
                        <p className="text-[9px] font-black uppercase leading-tight mb-0.5">{v.label}</p>
                        <p className="text-indigo-400 font-black text-sm">${v.price}</p>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <button
                  ...
                  className="w-full py-4 bg-white text-black font-black uppercase text-[10px] tracking-widest rounded-2xl flex items-center justify-center gap-2 hover:bg-indigo-400 transition-all"
                >
          ...
          <div className="flex items-center gap-3 bg-zinc-900 px-4 py-2 rounded-2xl border border-zinc-800">
            {user?.name && (
              <div className="w-7 h-7 rounded-full bg-indigo-500/20 border border-indigo-500/30 flex items-center justify-center">
                <span className="text-[10px] font-black text-indigo-400">{user.name[0].toUpperCase()}</span>
              </div>
            )}
            <div>
        ...
        <div className="max-w-6xl mx-auto px-8 flex gap-1">
          {[
            ...
            <button
              ...
              className={`flex items-center gap-2 px-5 py-3 text-[10px] font-black uppercase tracking-widest border-b-2 transition-all ${activeTab === key ? 'border-indigo-500 text-indigo-400' : 'border-transparent text-zinc-500 hover:text-zinc-300'}`}
            >
                    ...
                    <div className="aspect-[3/4] relative overflow-hidden bg-black">
                      ...
                      {design.is_private && (
                        <div className="absolute top-3 right-3 bg-black/60 backdrop-blur-sm px-2 py-1 rounded-full flex items-center gap-1">
                          ...
                          <span className="text-[7px] font-black text-indigo-400 uppercase">Private</span>
                        </div>
                      )}
                      ...
                      <div className="absolute inset-0 bg-black/75 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center gap-2 backdrop-blur-sm">
                        <button
                          ...
                          className="w-40 py-2.5 bg-white text-black text-[9px] font-black uppercase rounded-xl hover:bg-indigo-400 transition-all"
                        >
                          View & Download
                        </button>
                        <button
