...
export function ProductDetail() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      if (!slug) { setError("Invalid product link."); return; }

      try {
        const res = await fetch(`${API_URL}?action=get_product&slug=${encodeURIComponent(slug)}`);
  ...
  if (!product) return (
    <div className="min-h-screen bg-black flex items-center justify-center font-mono text-[10px] uppercase tracking-widest opacity-20 animate-pulse">
      Syncing Dimensions...
    </div>
  );
          ...
          <div className="space-y-12">
            <div>
              ...
              <div className="flex flex-wrap gap-3 mb-8">
                <span className="px-4 py-1.5 bg-zinc-900 border border-zinc-800 rounded-full text-[10px] font-black uppercase text-zinc-400 tracking-widest">
                  {product.category}
                </span>
            ...
            {selectedVariant.price >= 24 && (
              <div className="p-8 bg-zinc-900 rounded-[2.5rem] border border-zinc-800 flex items-center justify-between group">
                ...
                <button
                  ...
                  className={`w-14 h-7 rounded-full transition-all ${isFramed ? 'bg-emerald-600' : 'bg-zinc-700'} relative`}
                >
