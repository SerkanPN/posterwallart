...
// --- CONSTANTS (YOUR REQUESTED WARM ROOM) ---
...
export function SpecialForRoom() {
  ...
  // --- SELECTION STATES (ALL DROPDOWNS) ---
  ...
  useEffect(() => {
    const loadConfigs = async () => {
      console.log("[LOG] System: Syncing with public/config JSON files...");
      try {
  ...
  const analyzeRoom = async (base64Image: string) => {
    console.log("[LOG] API: Starting architectural scale analysis...");
    setIsAnalyzing(true);
    try {
      const response = await fetch(`/api/gemini`, {
        ...
        body: JSON.stringify({
          ...
          payload: {
            contents: [{ parts: [
              { text: 'Analyze room scale. Return ONLY JSON: { "ppi": number, "rotateY": number, "skewY": number, "detectedStyle": "string", "suggestedTheme": "string", "suggestedSubject": "string" }' },
  ...
  const handleCreateForMe = async () => {
    ...
    console.log("[LOG] Production: Master flow triggered");
    ...
    try {
      ...
      const subject = analysisData?.suggestedSubject || "art";
      ...
      try {
        ...
        console.log("[LOG] API: Gemini engine success");
      } catch (e) {
        console.warn("[LOG] API: Fallback to FLUX engine");
      ...
      if (!finalBase64) throw new Error("Cluster fail");
      ...
      formData.append('metadata', JSON.stringify({ seo_title: `Custom ${style} Masterpiece` }));
      ...
      if (result.success) {
        ...
        console.log("[LOG] System: Creation live");
      }
    } catch (error: any) {
      console.error("[ERROR] Flow failure:", error);
  ...
  const handleUpscaleAndDownload = async (product: Product) => {
    ...
    try {
      ...
      if (!data.upscaledUrl) throw new Error("Fail");
      const link = document.createElement('a');
      link.href = data.upscaledUrl;
      link.download = `HD_${product.id}.png`;
      link.target = '_blank';
    ...
    <div className="flex h-[calc(100vh-4rem)] bg-zinc-950 text-zinc-50 overflow-hidden font-sans">
      ...
      {modalProduct && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-zinc-900 border border-zinc-800 rounded-[40px] w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col md:flex-row shadow-2xl animate-in zoom-in-95 duration-300">
            ...
            <div className="flex-1 bg-zinc-950 flex items-center justify-center p-8"><img src={modalProduct.image} className="max-w-full max-h-full object-contain rounded-2xl shadow-2xl border border-zinc-800" alt="D" /></div>
            <div className="w-full md:w-[380px] p-10 flex flex-col justify-between border-l border-zinc-800">
              <div className="space-y-8">
                <div><p className="text-[10px] font-black uppercase tracking-[0.3em] text-emerald-500 mb-2">Masterpiece</p><h2 className="text-3xl font-black italic tracking-tighter uppercase leading-tight">{modalProduct.title}</h2></div>
      ...
      <div className={`w-[320px] border-r border-zinc-800 bg-zinc-950 flex flex-col h-full overflow-y-auto transition-all ${isInterfaceLocked ? 'opacity-40 pointer-events-none' : 'opacity-100'}`}>
        <div className="p-6 space-y-8 pb-24">
          <div><h1 className="text-xl font-black italic tracking-tighter text-emerald-500 uppercase leading-none">SPECIAL FOR<br/>YOUR ROOM</h1><p className="text-[9px] text-zinc-500 font-bold tracking-widest mt-2 uppercase opacity-50">Studio Hub v7.0</p></div>
      ...
      <div className="flex-1 p-6 flex flex-col gap-6 relative overflow-hidden bg-zinc-950">
        <div className="flex gap-4 h-32">
          ...
          <div {...refDrop.getRootProps()} className="flex-1 border-2 border-dashed border-zinc-800 rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:bg-zinc-900/50 transition-all group relative overflow-hidden bg-zinc-900/10">
            ...
            {refImage ? <div className="absolute inset-0"><img src={refImage} className="w-full h-full object-cover opacity-40" alt="Ref" /><button onClick={(e) => { e.stopPropagation(); setRefImage(null); }} className="absolute top-1 right-1 bg-red-500/80 p-1 rounded-md text-[7px] font-black">RESET</button></div> : null}
      ...
      <div className="w-[280px] border-l border-zinc-800 bg-zinc-950 flex flex-col h-full">
        <div className="p-6 space-y-6 overflow-y-auto scrollbar-hide">
          ...
          <div className="space-y-4">
            {recommendations.length === 0 && <p className="text-[9px] text-zinc-700 italic text-center py-10 font-bold uppercase tracking-tighter">Genesis Pending</p>}
